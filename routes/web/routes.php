...
use App\Http\Controllers\Payment_Methods\YookassaPaymentController;
...
if (!$isGatewayPublished) {
    Route::group(['prefix' => 'payment'], function () {
        ...
        Route::group(['prefix' => 'yookassa', 'as' => 'yookassa.'], function () {
            Route::get('pay', [YookassaPaymentController::class, 'index'])->name('pay');
            Route::post('payment', [YookassaPaymentController::class, 'payment'])->name('payment');
            Route::any('callback', [YookassaPaymentController::class, 'callback'])->name('callback')
                ->withoutMiddleware([VerifyCsrfToken::class]);
            Route::any('return', [YookassaPaymentController::class, 'return'])->name('return')
                ->withoutMiddleware([VerifyCsrfToken::class]);
            Route::any('webhook', [YookassaPaymentController::class, 'webhook'])->name('webhook')
                ->withoutMiddleware([VerifyCsrfToken::class]);
            Route::any('cancel', [YookassaPaymentController::class, 'cancel'])->name('cancel');
        });
    });
}
