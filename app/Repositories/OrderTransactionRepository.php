...
class OrderTransactionRepository implements OrderTransactionRepositoryInterface
{
    ...
    public function getListWhereBetween(array $filters = [], ?string $selectColumn = null, ?string $whereBetween = null, ?string $groupBy = null, array $whereBetweenFilters = [], array $relations = [], int|string $dataLimit = DEFAULT_DATA_LIMIT, ?int $offset = null): Collection|LengthAwarePaginator
    {
        return $this->orderTransaction->with($relations)->where($filters)
            ->when($selectColumn == 'seller_amount', function ($query) use ($groupBy) {
                $dateParts = $groupBy == 'month' ? dbDatePartsYearMonth('created_at') : dbDatePartsWithDayName('created_at');
                $query->select(
                    DB::raw(dbIfNull('sum(seller_amount)', 0) . ' as sums'),
                    DB::raw($dateParts)
                );
            })
            ->when($selectColumn == 'admin_commission', function ($query) use ($groupBy) {
                $dateParts = $groupBy == 'month' ? dbDatePartsYearMonth('created_at') : dbDatePartsWithDayName('created_at');
                $query->select(
                    DB::raw(dbIfNull('sum(admin_commission)', 0) . ' as sums'),
                    DB::raw($dateParts)
                );
            })
            ...
            ->when($groupBy == 'day', function ($query) use ($filters) {
                return $query->groupBy('year', 'month', 'day', 'day_of_week');
            })
    ...
    public function getCommissionEarningStatisticsData(?string $sellerIs = null, array $dataRange = [], array $groupBy = [], int $dateStart = 1, int $dateEnd = 12): array
    {
        ...
        $commissionEarnings = $this->orderTransaction
            ...
            ->select(DB::raw(dbIfNull('sum(admin_commission)', 0) . ' as sums'),
                DB::raw(dbDateParts('created_at'))
            )
    ...
    public function getEarningStatisticsData(?string $sellerIs = null, array $dataRange = [], array $groupBy = [], int $dateStart = 1, int $dateEnd = 12): array
    {
        ...
        $inhouseEarnings = $this->orderTransaction
            ...
            ->where(['status' => 'disburse'])
            ->select(DB::raw(dbIfNull('sum(seller_amount)', 0) . ' as sums'),
                DB::raw(dbDateParts('created_at'))
            )
            ->when($dataRange, function ($query) use ($dataRange) {
                return $query->whereBetween('created_at', [$dataRange['from'], $dataRange['to']]);
