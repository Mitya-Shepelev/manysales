...
class AIContentGeneratorService
{
    ...
    {
        $this->loadTemplates();
        $this->providers = [new OpenAIProvider(), new ClaudeProvider(), new OpenRouterProvider()];
    }
     ...
     */
    public function generateContent(string $contentType, mixed $context = null, ?string $langCode = null, ?string $description = null, ?string $imageUrl = null): string
    {
        // Use provided langCode, or fall back to current app locale, or session locale
        if (empty($langCode)) {
            $langCode = session('locale') ?? app()->getLocale() ?? 'en';
        }
